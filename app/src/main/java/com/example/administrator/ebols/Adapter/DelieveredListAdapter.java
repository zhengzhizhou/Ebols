...
public class DelieveredListAdapter extends RecyclerView.Adapter<DelieveredListAdapter.ViewHolder> {
    ...
    public List<DelieveredListObject> delieveredListObjects;
    public DelieveredListAdapter(List<DelieveredListObject> delieveredListObjects, Context context, String tab){
        this.delieveredListObjects = delieveredListObjects;
        this.context = context;
        this.tab = tab;
    }
    @Override
    public DelieveredListAdapter.ViewHolder onCreateViewHolder(final ViewGroup parent, int viewType) {
        final View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.listitem_archived, parent, false);

        final DelieveredListAdapter.ViewHolder viewHolder = new DelieveredListAdapter.ViewHolder(view);
        view.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                selectedPos =viewHolder.getAdapterPosition();
                Intent intent = new Intent(view.getContext(), LoadInfoActivity.class);
                intent.putExtra("id", delieveredListObjects.get(selectedPos).list_id);
    ...
    @Override
    public void onBindViewHolder(DelieveredListAdapter.ViewHolder holder, int position) {
        holder.itemAmount_textview.setText(delieveredListObjects.get(position).itemAmount);
        holder.IdLoaded_textview.setText(delieveredListObjects.get(position).list_id);
        holder.startPoint_textview.setText(delieveredListObjects.get(position).startPoint);
        holder.destination_textview.setText(delieveredListObjects.get(position).destination);
        holder.itemView.setSelected(selectedPos == position);
    }

    @Override
    public int getItemCount() {
        return delieveredListObjects.size();
    }
